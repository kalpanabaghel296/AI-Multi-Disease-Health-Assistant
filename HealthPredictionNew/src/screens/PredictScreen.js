...
const MODELS = [
  {
    ...
    // inputs: '8 clinical inputs', model: 'scikit-learn', 
    active: true, color: '#2563EB', bg: '#EFF6FF',
  },
  {
    icon: '❤️', name: 'Heart Disease',    route: 'Heart',
    desc: 'Assess cardiovascular risk using ECG results and lab parameters.',
    // inputs: '13 clinical inputs', model: 'Ensemble Model', 
    active: true, color: '#EF4444', bg: '#FEF2F2',
  },
  {
    icon: '🫁', name: 'Lung Cancer Risk', route: 'Lung',
    desc: 'Evaluate lung cancer risk from symptoms and lifestyle factors.',
    // inputs: '14 binary inputs', model: 'SVM + Ensemble', 
    active: true, color: '#8B5CF6', bg: '#e5eff3',
  },
  {
    icon: '🔬', name: 'Dermatosis',       route: 'Dermatosis',
    desc: 'CNN-based skin lesion classification from photos.',
    // inputs: 'Image upload', model: 'PyTorch CNN', 
    active: true, color: COLORS.textMuted, bg:'#edf3e0',
  },
  {
    icon: '🩻', name: 'Pneumonia',        route: 'Pneumonia',
    desc: 'Detect pneumonia from chest X-ray images.',
    inputs: 'X-Ray image',
     active: true, color: COLORS.textMuted, bg:'#cfeee0',
  },
];
...
const PredictScreen = ({ navigation }) => (
  <SafeAreaView style={s.safe} edges={['top']}>
    <View style={s.header}>
      ...
      <Text style={s.headerSub}>Select to begin AI assessment</Text>
    </View>
