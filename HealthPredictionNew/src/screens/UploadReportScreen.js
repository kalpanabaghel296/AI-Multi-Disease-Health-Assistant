...
const UploadReportScreen = () => {
  ...
  const pickImage = () => {
    launchImageLibrary(
      { mediaType: 'mixed', quality: 0.9, includeBase64: false },
      (resp) => {
        ...
        if (asset) {
          setFile(asset);
          setStep(2);
          setExtracted(null); setRawText(''); setPrediction(null);
        }
      }
    );
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      Alert.alert('Upload Failed', err.response?.data?.detail || 'Upload failed.');
  ...
  const handleExtract = async () => {
    ...
    try {
      const data = await reportService.extractReport();
