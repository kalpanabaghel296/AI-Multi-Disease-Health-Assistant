...
const DermatosisPredictionScreen = ({ navigation }) => {
  ...
  const handlePredict = async () => {
    ...
    setResult({
      result: 'Acne Keloidalis Nuchae',
      confidence: 91.3,
      all_probabilities: {
        'Acne Keloidalis Nuchae': 91.3,
        'Folliculitis':           4.2,
        'Psoriasis':              2.1,
        'Eczema':                 1.1,
        'Tinea Corporis':         0.8,
        'Seborrheic Dermatitis':  0.3,
        'Normal Skin':            0.2,
      },
      suggestions: [
        { icon: '🩺', title: 'Consult a Dermatologist', desc: 'Visit a skin specialist for proper diagnosis and treatment plan.' },
        { icon: '💊', title: 'Topical Antibiotics', desc: 'Clindamycin or benzoyl peroxide creams may help reduce inflammation.' },
        { icon: '🚿', title: 'Gentle Cleansing', desc: 'Use a mild, fragrance-free cleanser on the affected area twice daily.' },
        { icon: '🚫', title: 'Avoid Irritants', desc: 'Avoid tight collars, rough fabrics, and hair products near the neckline.' },
        { icon: '☀️', title: 'Sun Protection', desc: 'Apply SPF 30+ sunscreen to prevent post-inflammatory hyperpigmentation.' },
      ],
    });
          ...
          <View style={s.resultCard}>
            ...
            {result.suggestions && (
              <View style={s.suggestionsBox}>
                <Text style={s.suggestionsTitle}>💬 Recommendations</Text>
                {result.suggestions.map((item, index) => (
                  <View key={index} style={s.suggestionRow}>
...
const s = StyleSheet.create({
  ...
  // ✅ SUGGESTIONS STYLES
  suggestionsBox: { marginTop: SPACING.md, backgroundColor: COLORS.background, borderRadius: RADIUS.md, padding: SPACING.sm },
  suggestionsTitle: { fontSize: 13, fontWeight: '700', color: COLORS.textPrimary, marginBottom: SPACING.sm },
  suggestionRow: { flexDirection: 'row', gap: SPACING.sm, marginBottom: SPACING.sm, alignItems: 'flex-start' },
  suggestionIcon: { fontSize: 20, width: 28 },
  suggestionText: { flex: 1 },
  suggestionTitle: { fontSize: 13, fontWeight: '700', color: COLORS.textPrimary },
  suggestionDesc: { fontSize: 12, color: COLORS.textSecondary, lineHeight: 17, marginTop: 2 },
